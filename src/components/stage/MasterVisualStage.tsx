...
export default function MasterVisualStage() {
  ...
  useLayoutEffect(() => {
    ...
    const updateStageFromProgress = (p: number) => {
      ...
      if (canvasRef.current) {
        gsap.set(canvasRef.current, { scale: camPushScale, x: 0, y: 0 })
      }

      const targetFrame = mapProgressToFrame(p)
      ...
      const mode1Opacity = getHeroCinematicOpacity(p)
      if (canvasWrapRef.current) {
        gsap.set(canvasWrapRef.current, { opacity: mode1Opacity })
      }

      const stillOpacity = getHeroStillOpacity(p)
      if (heroStillRef.current) {
        gsap.set(heroStillRef.current, { opacity: stillOpacity })
      }
    }

    // Synchronous progress listener from Hero scroll driver (SINGLE SOURCE OF TRUTH)
    const unsubProgress = subscribeHeroProgress((p) => updateStageFromProgress(p))

    // Initial render setup for current stage progress
    updateStageFromProgress(getHeroProgress())
